...
- [x] Two projects can coexist on one test node.
- [x] Tenant A cannot connect/access Tenant B.
- [x] Customer role is not DB owner/superuser.
- [x] Retry works after simulated failures.
- [x] Project reaches its terminal state only after validation. In Phase 02 that state is
      `PROVISIONED`; Phase 03 carries a project from there to `ACTIVE` once it has API workers
      and a route.
- [x] Cleanup never drops a possibly-used DB automatically.
...
- [x] No customer-reachable role is a member of `maludb` or of any `BYPASSRLS` role.
- [x] Plan resource settings are applied to the authenticator login role, scoped `IN DATABASE`, and verified to take effect through `SET ROLE`.
- [x] The negative tests in `specs/tenant-role-model.md` pass, except J (free-tier projects
      have no route reachable from outside the gateway), which needs the gateway that Phase 03
      introduces and is carried to that phase.
...
- [x] A failed provisioning run leaks no credential into `provisioning_jobs.error_detail` or any log.
