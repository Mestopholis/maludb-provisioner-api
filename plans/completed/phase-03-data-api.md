...
- 2026-08-15 — Slice 4 complete: the proof milestone. `@supabase/supabase-js`
  ...
  hostname that actually resolves. Fourteen cases pass, including the three
  Phase 00 findings as regressions -- `.upsert()` against
  `GENERATED BY DEFAULT AS IDENTITY`, a table created after the worker started
  being visible without a restart, and `gen_salt` *not* being callable as RPC.
  The suite immediately found a real gap slice 3 had left: the gateway proxied
  `/rest/v1/...` verbatim and PostgREST answered PGRST125, because the surface
  prefix belongs to the gateway and Supabase strips it too. Compatibility
  matrix entries move off `planned` only for what the suite covers, each
  carrying the case name that earned it. 279 tests.
