...
- **~~Which vector search surface, for whom?~~** **MaluDB's vector compartments**, through platform-owned definer wrappers in `maludb`, to `service_role` only, on every plan with per-plan vector limits, opt-in per project, embeddings as pgvector `vector`, exact search first and ANN after measurement — ADR-077, 2026-09-13. pgvector search already worked (ADR-076). Plan: `plans/active/phase-12-vector-compartments.md`.
- **Do moves and per-tenant restores lose `maludb_core` data?** Yes, as found 2026-09-13 (ADR-077 context): `maludb_core` registers no table with `pg_extension_config_dump`, so `pg_dump` — which tenant moves (ADR-066) and per-tenant restores (ADR-059) are built on — carries none of its table data. ADR-077 decision 8 gates vector compartments on carrying `malu$vector_*` rows beside the dump. **Still open for everything else `maludb_core` stores**: the data-model graph survives because its customer-facing copy lives in ordinary tables and a refresh rebuilds the rest, but the memory pipeline would lose customer data. Needs the upstream report (register data tables for dump) and a platform answer before the memory pipeline is exposed.
